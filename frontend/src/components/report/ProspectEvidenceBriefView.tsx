  ...
  return <>
    <section className="mt-6 rounded-card border border-line-soft bg-card p-5">
      <div className="flex flex-wrap items-start justify-between gap-4">
        ...
        <span className={'rounded-control border px-2 py-1 font-ui text-sm font-semibold ' + verdictStyle[aggregate]}>{displayLabel(aggregate).toUpperCase()}</span>
      </div>
    ...
    {brief.contacts.length > 0 && <Section title="Contact paths"><div className="grid gap-3 sm:grid-cols-2">{brief.contacts.map((contact) => <div key={`${contact.contact_type}-${contact.value}`} className="rounded-control border border-line-soft bg-card p-3"><p className="label-caps text-ink-faint">{displayLabel(contact.contact_type)}</p><a href={contactHref(contact.contact_type, contact.value)} target={['email', 'phone'].includes(contact.contact_type) ? undefined : '_blank'} rel="noreferrer" className="mt-1 block break-all text-sm text-action hover:text-ink">{contact.value}</a><p className="mt-1 text-xs text-ink-faint">{displayLabel(contact.state)}</p></div>)}</div></Section>}
